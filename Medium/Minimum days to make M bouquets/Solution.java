...
class Solution {
    public static void main(String args[]) {
        ...
        System.out.println("Enter no of adjacent flowers to choose: ");
        int k = sc.nextInt();

        System.out.println("Enter number of bouquets you need: ");
        int m = sc.nextInt();

        System.out.println("Minimum no of days required: "+minDaysBloom(arr, k, m));
    ...
    public static int minDaysBloom(int[] arr, int k, int m) {
        ...
        while(low<=high){
            ...
            if(bouquets(arr, mid, k, m)<m)low=mid+1;
            else high=mid-1;
        }
        return low;
    }
    static int bouquets(int arr[], int day, int k, int m){
        int bloomed = 0, noOfBouquets = 0;
        
        for(int ele: arr){
            if(day>=ele)bloomed++;
            else{
                noOfBouquets+=bloomed/k;
                bloomed=0;
            }
        }
        noOfBouquets+=bloomed/k;
