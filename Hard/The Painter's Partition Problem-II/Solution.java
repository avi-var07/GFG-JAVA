...
class Solution {
    public static void main(String args[]) {
        ...
        int k = sc.nextInt();
        System.out.println("Min time is: "+minTime(arr, k));
        sc.close();
    }
    ...
    public static int minTime(int[] arr, int k) {
        ...
        while(maxi<=sum){
            ...
            if(canAllocate(arr, mid)>k)maxi = mid+1;
            else sum = mid-1;
        }
        return maxi;
    }
    static int canAllocate(int arr[], int time){
        int painter =1, lastAlloted = 0;
        for(int i = 0; i<arr.length; i++){
            if(arr[i]+lastAlloted<=time)lastAlloted+=arr[i];
            else{
                painter++;
                lastAlloted=arr[i];
            }
