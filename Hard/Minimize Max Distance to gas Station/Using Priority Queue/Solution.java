...
class Solution {
    ...
    public static double findSmallestMaxDist(int stations[], int k) {
        ...
        for(int i=0;i<stations.length-1;i++)pq.add(new Pair(stations[i+1]-stations[i], i));
        
        for(int i = 1;i<=k;i++){
            Pair tp = pq.poll();
            int section = tp.second;
            
            howMany[section]++;
            
            double diff  = stations[section+1]-stations[section];
            double newSection = diff/(double)(howMany[section]+1);
            
            pq.add(new Pair(newSection, section));
        }
